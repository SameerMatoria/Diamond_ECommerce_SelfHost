...
export default function Checkout() {
  ...
  if (!token) {
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
        ...
        <p className="mt-2 text-sm text-slate-300">Sign in to checkout.</p>
      </div>
    );
  }
  ...
  if (cart && cart.items?.length === 0) {
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
        <h2 className="text-2xl font-semibold">Checkout</h2>
        <p className="mt-2 text-sm text-slate-300">Your cart is empty.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
        ...
        />
        <input
          className="md:col-span-2 rounded-xl border border-slate-700 bg-slate-900/60 px-4 py-3"
        ...
        <input
          ...
          required
        />
        <button className="md:col-span-2 rounded-full bg-brand-500 px-6 py-3 text-sm font-medium text-white">
          Place order (COD)
        </button>
      </form>
    </div>
